...
export default function TaskInbox({ tasks, onUpdateTask, onDeleteTask }) {
  ...
  // Drag and drop handlers - matching TaskQueue exactly
  ...
  const handleDragEnd = () => {
    if (draggedIndex !== null && overIndex !== null && draggedIndex !== overIndex) {
      // Reorder and update priorities
      const newTasks = [...sortedTasks];
      const [removed] = newTasks.splice(draggedIndex, 1);
      newTasks.splice(overIndex, 0, removed);
      
      // Update priorities based on new positions (top = highest priority)
      newTasks.forEach((task, idx) => {
        const newPriority = Math.max(1, 4 - Math.floor(idx * 4 / newTasks.length));
  ...
  // Calculate transform for visual slot opening effect - matching TaskQueue
        ...
        {sortedTasks.map((task, index) => {
          ...
          return (
            <div
              ...
              className={`mb-3 select-none ${isDragging ? "opacity-40" : ""}`}
              style={{
                transform: transform,
            ...
            >
              ...
              >
                <div className="flex items-start gap-3">
                  {/* Drag handle */}
                  <div className="flex-shrink-0 cursor-grab active:cursor-grabbing text-muted-foreground hover:text-foreground pt-1">
